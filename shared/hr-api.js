...
(function () {
  ...
  function cycleMonthOf(dateStr) {
    const t = new Date((dateStr || bkkToday()) + 'T00:00:00');
    const end = (t.getDate() <= 20) ? new Date(t.getFullYear(), t.getMonth(), 20)
                                     : new Date(t.getFullYear(), t.getMonth() + 1, 20);
  ...
  async function dispatch(p) {
    try {
      ...
      switch (p.action) {
        ...
        case 'hr_taskset_get':       return await hrTaskSetGet(p);
        case 'hr_taskset_save':      return await hrTaskSetSave(p);
        ...
        case 'hr_qa_folder_update':  return await hrQaFolderUpdate(p.data);
  ...
  async function hrScoreIssueWarnings(which) {
    // ★ ระบบวินัยใหม่ (สะสม/บันได · ต้องมีหลักฐานทุกขั้น) — เลิก "ออกใบเตือนอัตโนมัติจากคะแนน" แล้ว
    //   เพราะจะข้ามขั้นวาจา/ลายลักษณ์ และไม่มีหลักฐานแนบ · ให้ทำทีละขั้นที่หน้าวินัยแทน
    return { ok: false, error: 'ระบบวินัยแบบสะสม/บันไดไม่ออกใบเตือนอัตโนมัติจากคะแนนแล้ว — ต้องดำเนินการทีละขั้น (ตักเตือนวาจา → ลายลักษณ์อักษร → ออกใบเตือน) พร้อมแนบหลักฐานทุกขั้น ที่หน้า "วินัย" · ระบบจะเลื่อนขั้นสะสมให้เอง' };
    // (เก็บโค้ดเดิมไว้ด้านล่างเผื่อย้อน แต่ไม่ทำงานแล้ว)
  ...
  function _salesDate(text, fallbackIso) {
    ...
    if (m) {
      ...
      if (d >= 1 && d <= 31 && mo >= 1 && mo <= 12 && y >= 2000 && y <= 2100) { const p = n => String(n).padStart(2, '0'); return y + '-' + p(mo) + '-' + p(d); }
    }
    return (fallbackIso || '').slice(0, 10) || null;   // วันที่ในข้อความเพี้ยน → ใช้วันที่ส่ง
  }
  ...
  async function hrLineBackfill(p) {
    ...
    // ---- Pass 2: แยกยอดขายจากข้อความในกลุ่มที่ผูกสาขาทั้งหมด (ไล่ตรง ๆ ไม่ง้อ ilike) ----
    const mappedGids = Object.keys(grpBranch);
    const salesMap = {}; let scanned = 0, hit = 0;
    if (mappedGids.length) {
      let from = 0, page = 1000;
      for (;;) {
        const { data: sms, error } = await sb().from('line_messages')
          .select('group_id,branch_id,display_name,text,sent_at')
          .in('group_id', mappedGids)
        ...
        scanned += sms.length;
        for (const m of sms) {
          const bid = grpBranch[m.group_id] || m.branch_id || null; if (!bid) continue;
  ...
  async function hrAdvanceCreate(d, auth) {
    ...
    const { data: br } = await sb().from('branches').select('name').eq('branch_id', emp.branch_id || '').maybeSingle();
    const month = d.cycle_month || cycleMonthOf();   // ดีฟอลต์ = เดือนสิ้นรอบ 21–20 (ไม่ใช่เดือนปฏิทิน)
  ...
  // ---------- TASKS v2: ตั้งค่างานในกะรายสาขา ----------
  //   HR  : แก้ได้ทุกอย่าง (เนื้อหางาน · เปิด-ปิด · รอบ/วัน · กะ)
  //   ผจก.: แก้ได้เฉพาะ "งานนี้ให้ผลัดไหนทำ" + "กำหนดวัน" ของสาขาตัวเองเท่านั้น
  async function _tsActor(auth) {
    auth = auth || {};
    if (!auth.mgr_emp) return { role: 'hr', branch_id: null, emp_id: null, name: 'สำนักงาน (HR)' };
    const { data } = await sb().rpc('mgr_login', { p_emp_id: String(auth.mgr_emp), p_pin: String(auth.mgr_pin || '') });
    const row = Array.isArray(data) ? data[0] : data;
    if (!row || !row.branch_id) return { role: 'invalid', branch_id: null, emp_id: null, name: '' };
    return { role: 'mgr', branch_id: String(row.branch_id), emp_id: String(auth.mgr_emp), name: (row.nickname || row.name || 'ผจก.') };
  }
  const _tsArr  = v => Array.isArray(v) ? v : [];
  const _tsInts = v => _tsArr(v).map(x => parseInt(x, 10)).filter(x => !isNaN(x));

  async function hrTaskSetGet(p) {
    const actor = await _tsActor(p);
    if (actor.role === 'invalid') return { ok: false, error: 'PIN ไม่ถูกต้อง' };
    const branch = (actor.role === 'mgr') ? actor.branch_id : String(p.branch || '');
    const { data: defs, error } = await sb().from('task_defs').select('*');
    if (error) throw error;
    let brs = [];
    try { const b = await sb().from('branches').select('branch_id,name,task_v2').order('branch_id'); brs = b.data || []; }
    catch (e) { const b = await sb().from('branches').select('branch_id,name').order('branch_id'); brs = b.data || []; }
    let ov = [], dates = [];
    if (branch) {
      const o = await sb().from('task_def_branches').select('*').eq('branch_id', branch);
      ov = o.data || [];
      const m = String(p.month || '').match(/^(\d{4})-(\d{2})$/);
      if (m) {
        const d = await sb().from('task_def_dates').select('*').eq('branch_id', branch)
          .gte('work_date', m[1] + '-' + m[2] + '-01').lte('work_date', m[1] + '-' + m[2] + '-31');
        dates = d.data || [];
      }
    }
    const rows = (defs || []).slice().sort((a, b) =>
      ((a.step == null ? 99 : a.step) - (b.step == null ? 99 : b.step)) || ((a.sort || 0) - (b.sort || 0)) || (a.id - b.id));
    return { ok: true, role: actor.role, branch, rows, branches: brs, overrides: ov, dates };
  }

  // แก้ "ค่ากลาง" ของงาน — HR เท่านั้น
  async function hrTaskSetSave(p) {
    const actor = await _tsActor(p);
    if (actor.role !== 'hr') return { ok: false, error: 'เปิด-ปิด/แก้ไขงาน ทำได้เฉพาะฝั่ง HR' };
    const d = p.data || {};
    if (!d.id) return { ok: false, error: 'ไม่พบงาน' };
    const row = {};
    ['title', 'how_to', 'criteria', 'photo_hint', 'source_ref', 'qssi_cat', 'who_label', 'link_kind', 'group_kind', 'manual_note']
      .forEach(k => { if (k in d) row[k] = (d[k] === '' ? null : d[k]); });
    if ('freq' in d) row.freq = ['daily', 'weekly', 'monthly'].includes(d.freq) ? d.freq : 'daily';
    if ('days_of_week' in d) row.days_of_week = _tsInts(d.days_of_week);
    if ('day_of_month' in d) row.day_of_month = _tsInts(d.day_of_month);
    if ('shift_ids' in d)    row.shift_ids    = _tsArr(d.shift_ids).map(String);
    ...
    if (!Object.keys(row).length) return { ok: false, error: 'ไม่มีข้อมูลให้บันทึก' };
    const { error } = await sb().from('task_defs').update(row).eq('id', d.id);
    if (error) throw error;
    return { ok: true };
  }

  // ตั้งค่าเฉพาะสาขา — HR ตั้งได้ทุกช่อง · ผจก. ตั้งได้เฉพาะ shift_ids / วัน
  async function hrTaskSetBranch(p) {
    const actor = await _tsActor(p);
    if (actor.role === 'invalid') return { ok: false, error: 'PIN ไม่ถูกต้อง' };
    const d = p.data || {};
    const branch = (actor.role === 'mgr') ? actor.branch_id : String(d.branch_id || p.branch || '');
    if (!branch) return { ok: false, error: 'ต้องเลือกสาขาก่อน' };
    if (!d.task_def_id) return { ok: false, error: 'ไม่พบงาน' };
    const row = {
      task_def_id: Number(d.task_def_id), branch_id: branch,
      updated_by: actor.emp_id || 'hr', updated_role: actor.role, updated_at: new Date().toISOString(),
    };
    if ('shift_ids' in d)    row.shift_ids    = (d.shift_ids === null)    ? null : _tsArr(d.shift_ids).map(String);
    if ('days_of_week' in d) row.days_of_week = (d.days_of_week === null) ? null : _tsInts(d.days_of_week);
    if ('day_of_month' in d) row.day_of_month = (d.day_of_month === null) ? null : _tsInts(d.day_of_month);
    if ('note' in d)         row.note         = d.note || null;
    if ('active' in d) {
      if (actor.role !== 'hr') return { ok: false, error: 'เปิด-ปิดงาน ทำได้เฉพาะฝั่ง HR' };
      row.active = (d.active === null) ? null : !!d.active;
    }
    const { error } = await sb().from('task_def_branches').upsert(row, { onConflict: 'task_def_id,branch_id' });
    if (error) throw error;
    return { ok: true };
  }

  // ปฏิทิน: ระบุวันที่เพิ่มงาน (add) หรือข้ามงาน (skip) เฉพาะวัน — HR และ ผจก. ทำได้
  async function hrTaskSetDate(p) {
    const actor = await _tsActor(p);
    if (actor.role === 'invalid') return { ok: false, error: 'PIN ไม่ถูกต้อง' };
    const d = p.data || {};
    const branch = (actor.role === 'mgr') ? actor.branch_id : String(d.branch_id || p.branch || '');
    if (!branch || !d.task_def_id || !d.work_date) return { ok: false, error: 'ข้อมูลไม่ครบ' };
    if (d.remove) {
      const { error } = await sb().from('task_def_dates').delete()
        .eq('task_def_id', Number(d.task_def_id)).eq('branch_id', branch).eq('work_date', d.work_date);
      if (error) throw error;
      return { ok: true };
    }
    const row = {
      task_def_id: Number(d.task_def_id), branch_id: branch, work_date: d.work_date,
      mode: (d.mode === 'skip' ? 'skip' : 'add'), note: d.note || null, created_by: actor.emp_id || 'hr',
    };
    const { error } = await sb().from('task_def_dates').upsert(row, { onConflict: 'task_def_id,branch_id,work_date' });
    if (error) throw error;
    return { ok: true };
  }

  // เปิด/ปิดการใช้แคตตาล็อกใหม่ (v2) รายสาขา — HR เท่านั้น
  async function hrTaskSetV2Flag(p) {
    const actor = await _tsActor(p);
    if (actor.role !== 'hr') return { ok: false, error: 'เฉพาะ HR เท่านั้น' };
    const d = p.data || {};
  ...
  async function hrQaFolderList() {
    ...
    const asgIdByF = {}; (asgR.data || []).forEach(a => { (asgIdByF[a.folder_id] = asgIdByF[a.folder_id] || []).push(a.emp_id); });
    ...
    const rows = folders.map(f => ({ ...f, assignees: asgByF[f.id] || [], assignee_ids: asgIdByF[f.id] || [], branch_ids: [...(asgBrByF[f.id] || [])], stats: itByF[f.id] || { total: 0, on_shelf: 0, sold: 0, removed: 0, expiring: 0 } }));
    return { ok: true, rows };
  }

  async function hrQaFolderUpdate(d) {
    d = d || {};
    if (!d.id) return { ok: false, error: 'ไม่ระบุโฟลเดอร์' };
    if (!d.title || !String(d.title).trim()) return { ok: false, error: 'ต้องระบุชื่อหัวข้อ' };
    const upd = { title: String(d.title).trim(), target_month: (d.target_month || '').trim() || null, note: (d.note || '').trim() || null };
    const { error } = await sb().from('qa_folders').update(upd).eq('id', d.id);
    if (error) return { ok: false, error: error.message };
    if (Array.isArray(d.emp_ids)) {                          // แก้ผู้รับผิดชอบ (แทนที่ทั้งหมด)
      if (!d.emp_ids.length) return { ok: false, error: 'เลือกผู้รับผิดชอบอย่างน้อย 1 คน' };
      await sb().from('qa_folder_assignees').delete().eq('folder_id', d.id);
      const { data: emps } = await sb().from('employees').select('emp_id,branch_id').in('emp_id', d.emp_ids);
      const brOf = {}; (emps || []).forEach(e => { brOf[e.emp_id] = e.branch_id; });
      const { error: e2 } = await sb().from('qa_folder_assignees').insert(d.emp_ids.map(id => ({ folder_id: d.id, emp_id: id, branch_id: brOf[id] || null })));
      if (e2) return { ok: false, error: e2.message };
    }
    await logAct('แก้ไขโฟลเดอร์ QA', null, upd.title);
    return { ok: true };
  }
